...
from semilearn.nets.utils import load_checkpoint, load_checkpoint_totally
...
class WideResNetCBN(nn.Module):
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        ...
        result_dict = {'logitsH':logitsH, 'logitsM':logitsM, 'logitsT':logitsT, 'logits':logits, 'feat':out}
        return result_dict
...
def wrn_28_2_cbn(pretrained=False, pretrained_path=None, **kwargs):
    model = WideResNetCBN(first_stride=1, depth=28, widen_factor=2, **kwargs)
    if pretrained:
        print('!'*100)
        ...
        model = load_checkpoint_totally(model, pretrained_path)
    return model


def wrn_28_8_cbn(pretrained=False, pretrained_path=None, **kwargs):
    model = WideResNetCBN(first_stride=1, depth=28, widen_factor=8, **kwargs)
    if pretrained:
        model = load_checkpoint(model, pretrained_path)
    return model
